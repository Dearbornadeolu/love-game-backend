...
class GameServer {
    ...
    handleGameMove(ws, data) {
        ...
        let winningCells = [];
        const winner = this.checkWinner(room.gameState.board, row, column, player.playerNumber, winningCells);
        if (winner) {
        ...
        this.broadcastToRoom(player.roomId, {
          ...
          winningCells: winningCells, // Include winning cells in the message
        });
      
        console.log(`${player.username} made move in column ${column}`);
      }
      
      // Modified checkWinner to return winning cells
      checkWinner(board, row, col, player, winningCells) {
        const directions = [
          [0, 1], [1, 0], [1, 1], [1, -1],
        ];
